...
The RPM spec file provided here is for Testcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://testcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Testcoin git repository](https://github.com/testcoin/testcoin) but are not part of
...
Currently only `Source10` is used, for the example `testcoin.conf` file.
...
in the testcoin git repository. At present only an SVG version of the Testcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the testcoin
...
needed when 0.12.1 is released, a proper fix is already in the Testcoin git
master branch.
...
idea to build Testcoin Core against a static version of that library compiled
...
The `testcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `testcoin-server` package. At this point, most RPM based
...
The `testcoin-server` package should have SELinux support. How to properly do
...
Testcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/testcoin/). He has been
packaging Testcoin for Fedora at least since 2012.
...
4. I renamed the `testcoin` package that contains the Qt GUI to `testcoin-core` as
that appears to be how the general population refers to it, in contrast to
`testcoin-xt` or `testcoin-classic`. I wanted to make sure the general population
