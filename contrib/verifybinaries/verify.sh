...
###   testcoincore.org and testcoin.org and compares them.
...
WORKINGDIR="/tmp/testcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://testcoincore.org"
HOST2="https://testcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="testcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'testcoin-',
...
if [ $? -ne 0 ]; then
   echo "testcoin.org failed to provide signature file, but testcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "testcoin.org and testcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Testcoin Core binary release signing key installed?"
   fi
