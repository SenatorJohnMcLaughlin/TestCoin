UNIX BUILD NOTES
====================
Some notes on how to build Testcoin Core in Unix.
...
memory available when compiling Testcoin Core. On systems with less, gcc can be
...
**For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~testcoin/+archive/testcoin).
...
See the section "Disable-wallet mode" to build Testcoin Core without wallet.
...
`--with-incompatible-bdb` according to the [PKGBUILD](https://projects.archlinux.org/svntogit/community.git/tree/testcoin/trunk/PKGBUILD).
As mentioned above, when maintaining portability of the wallet between the standard Testcoin Core distributions and independently built
...
with 4.8-built Testcoin Core is needed follow the steps under "Berkeley DB" above.
