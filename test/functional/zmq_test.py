...
from test_framework.test_framework import TestcoinTestFramework, SkipTest
from test_framework.util import (assert_equal,
                                 bytes_to_hex_str,
                                 hash256,
                                )

class ZMQTest (TestcoinTestFramework):
    ...
    def setup_nodes(self):
        ...
        # Check that testcoin has been built with ZMQ enabled
        ...
        if not config["components"].getboolean("ENABLE_ZMQ"):
            raise SkipTest("testcoind has not been built with zmq enabled.")
