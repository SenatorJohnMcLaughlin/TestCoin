...
const int TESTCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString TESTCOIN_IPC_PREFIX("testcoin:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/testcoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/testcoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/testcoin-paymentrequest";
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(TESTCOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(TESTCOIN_IPC_CONNECT_TIMEOUT);
...
//
// OSX-specific way of handling testcoin: URIs and PaymentRequest mime types.
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom testcoin addresses are not supported
        ...
        // Testcoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
