#!/bin/bash
# Copyright (c) 2013 The Testcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Testcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Testcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Testcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Testcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Testcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Testcoin 0.8 datadir."
    ;;
esac
